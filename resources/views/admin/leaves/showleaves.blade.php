...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0">Nghỉ phép cá nhân</h1>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{ url('employee_leaves') }}">Chấm công</a></li>
          <li class="breadcrumb-item active">Nghỉ phép cá nhân</a></li>
        </ol>
      </div>
    </div>
  </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="text-right">
                            <button type="button" onclick="window.location.href='{{route('leaves')}}'" class="btn btn-info btn-rounded" title="Apply for Leave"><i class="fas fa-plus"></i><span class="d-none d-xs-none d-sm-inline d-md-inline d-lg-inline"> Xin nghỉ phép</span></button>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($leave->status == 'pending') 
                                                    <div class="text-white badge badge-secondary font-weight-bold">Chưa giải quyết<textarea name="" id="" cols="30" rows="10"></textarea></div> 
                                                @endif
                                                @if($leave->status == 'Approved') 
                                                    <div class="text-white badge badge-success font-weight-bold">Đã duyệt</div> 
                                                @endif
                                                @if($leave->status == 'Declined') 
                                                    <div class="text-white badge badge-danger font-weight-bold">Chưa duyệt</div> 
                                                @endif
                                            </td>
                                            @if(Auth::user()->isAllowed('LeaveController:show') || Auth::user()->isAllowed('LeaveController:edit') || Auth::user()->isAllowed('LeaveController:destroy'))
                                                <td class="row">
